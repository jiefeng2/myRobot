...
CThermometer::CThermometer(QWidget *parent) : QWidget(parent)
{
    ...
    m_valueAnimation->setDuration(3000); //动画时间1秒
...
*/
    ...
    {
        //左侧刻度
        QPointF basePoint = m_rect.bottomLeft()+ QPointF(-2,-perHeight*i);  //+ QPointF(0,-perHeight/2)
        ...
        //右侧刻度
        basePoint = m_rect.bottomRight()+ QPointF(2,-perHeight*i)  ;  //+ QPointF(0,-perHeight/2)
...
/*
    ...
    QRectF flagRect = QRectF(m_rect.topLeft(),QSizeF(fMetrics.width(QString("ml")),fMetrics.height()));
    flagRect.adjust(-flagRect.width() - 20,-flagRect.height(),-flagRect.width() - 20,-flagRect.height());
    painter.drawText(flagRect,Qt::AlignCenter,QString("ml"));
    painter.restore();
*/
}
